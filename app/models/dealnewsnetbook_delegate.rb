class DealnewsnetbookDelegate < ActiveRecord::Base
	require 'rubygems'
	require 'hpricot'
	require 'open-uri'
	require 'uri'
...
def self.get_breaking_news
	doc = Hpricot.XML(open("http://dealnews.com/rss/535"))
	...
	breaking_news = (doc/"item").map do |item|
	  ...
	  temp_deal.buy_link = get_buy_link( (item/"link").inner_html ).to_my_utf8
	  ...
	  temp_deal.category = "netbook"
	  temp_deal.publish_date = DateTime.parse((item/"pubDate").inner_html)

	  #Stash if you make less than $30 on reselling. Flip if you make more than $30 on reselling.
	  if temp_deal.profit_margin < 30
	  temp_deal.stashflip_status = "stash"
  elsif temp_deal.profit_margin >= 30
